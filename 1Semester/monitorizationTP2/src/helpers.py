...
def percentage():
    return psutil.cpu_times_percent(interval=0.1)
...
def memory_usage():
    mem = psutil.virtual_memory()
    capacidade = round((mem.used + mem.inactive) / (1024 * 1024 * 1024), 2)  # GB
    return capacidade
...
def disk_usage():
    ...
    return round((disco.used + disco.free) / (1024 * 1024 * 1024), 2) # GB


def disk_total():
    disco = psutil.disk_usage('.')
    return round((disco.total + disco.free) / (1024 * 1024 * 1024), 2) # GB
